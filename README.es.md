...
ordena por score. Lo armamos originalmente con la ayuda de [Claude Code](https://claude.com/claude-code)
para una búsqueda puntual, y lo empaquetamos para que cualquiera pueda
adaptarlo a la suya.

## Origen de este proyecto

Esto no es una técnica de scraping inventada de cero. Es una **combinación**
de enfoques públicos y conocidos para cada fuente — los endpoints de ATS son
APIs documentadas por sus propios proveedores; el patrón de scraping del
endpoint guest de LinkedIn es algo que circula hace años en la comunidad —
**más un conjunto de adaptaciones propias** que fueron surgiendo de usarlo
en una búsqueda real: la detección de "híbrido disfrazado de remoto", el
manejo de blacklist entre corridas para no repetir avisos, la
deduplicación por título+empresa, y el sistema de scoring 100% configurable
por YAML. El valor de este repo está en haber empaquetado todo eso en algo
reutilizable y fácil de adaptar, no en la técnica de scraping en sí (que es
de dominio público).
...
Copiá `config.example.es.yaml` a `config.yaml` y poné tu CV en esta carpeta
(por ejemplo `mi_cv.pdf`). (Si preferís la plantilla en inglés, usá
`config.example.en.yaml` — el contenido y las claves son idénticas, solo
cambian los comentarios.)
...
├── README.md                  # landing bilingüe
├── README.es.md               # este archivo
├── README.en.md               # versión en inglés
├── config.example.es.yaml     # plantilla de config en español
├── config.example.en.yaml     # plantilla de config en inglés
...
El código (`.py`) tiene comentarios en inglés, siguiendo la convención
habitual en proyectos open source — solo la documentación (README, config)
está duplicada en ambos idiomas.
...
| LinkedIn (jobs-guest) | ⚠️ Funciona, pero fuera de ToS | Ver advertencia arriba y abajo. |
...
- Un problema real que tuvimos usando esto: el resumen de búsqueda de
  LinkedIn puede decir "remote" para un aviso que en realidad es híbrido o
  presencial. Si te importa la precisión, no confíes solo en el resultado de
  `main.py` — abrí cada URL final y fijate la descripción completa antes de
  aplicar. `filters.verify_remote_from_full_text()` está para automatizar
  ese segundo paso si querés (fetcheás cada URL, le pasás el texto, y te
  dice si el remote es real).
...
Pull requests bienvenidas, sobre todo para implementar alguno de los
portales regionales que quedaron como stub, o para agregar fuentes nuevas.
